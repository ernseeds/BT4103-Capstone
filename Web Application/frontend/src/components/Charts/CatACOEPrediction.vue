<template>
  <a-card
    title="COE Premium Forecast (Cat A)"
    :bordered="false"
    class="h-full shadow-md rounded-lg"
  >
    <!-- Top-right filters (same look as COEChart) -->
    <template #extra>
      <div class="filters">
...
export default {
  ...
  methods: {
    ...
    // user changes filters → rebuild using existing rows
    fetchData() {
    ...
    buildFromRows(allRows) {
      // Historical (same style as before): Premium present, Cat A only
      ...
      histRows = histRows
        .filter((r) => r.Vehicle_Class === "Category A" && r.Bidding_Date)
      ...
      // Forecast: Cat A from lastHistDate → +forecastYears
      let fcRows = allRows
        .filter((r) => r.Vehicle_Class === "Category A" && r.Bidding_Date)
        .sort((a, b) => a.Bidding_Date.localeCompare(b.Bidding_Date));
    ...
    renderChart(labels, hist, fc, ciLo, ciHi) {
      ...
      if (!this.firstRendered) {
        this.$emit("loaded", "CatAPred");
        this.firstRendered = true;
      }
    },
  },
  mounted() {
    // parent usually passes rows; if not yet, watch() will catch it
